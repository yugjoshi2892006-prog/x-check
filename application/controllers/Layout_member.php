...
class Layout_member extends CI_Controller
{
    ...
    public function update_layout_plan($id)
    {
        ...
        if (!empty($_FILES['drawing_file']['name'])) {
            ...
            if ($this->upload->do_upload('drawing_file')) {
                $new_drawing = $this->upload->data('file_name');
                $uploaded_paths[] = $config['upload_path'] . $new_drawing;
                $drawing = $new_drawing;
            } else {
                $this->_cleanup_uploaded_files($uploaded_paths);
                $this->session->set_flashdata('error', 'Store Side Drawing: ' . $this->upload->display_errors('', ''));
        ...
        if (!empty($_FILES['layout_photo']['name'])) {
            ...
            if ($this->upload->do_upload('layout_photo')) {
                ...
                $uploaded_paths[] = $config['upload_path'] . $new_photo;
                $photo = $new_photo;
            } else {
                $this->_cleanup_uploaded_files($uploaded_paths);
                $this->session->set_flashdata('error', 'Site Layout Photo: ' . $this->upload->display_errors('', ''));
        ...
        if (!empty($_FILES['soil_test_pdf']['name'])) {
            ...
            if ($this->upload->do_upload('soil_test_pdf')) {
                $new_soil = $this->upload->data('file_name');
                $uploaded_paths[] = $config['upload_path'] . $new_soil;
                $soil = $new_soil;
            } else {
                $this->_cleanup_uploaded_files($uploaded_paths);
                $this->session->set_flashdata('error', 'Soil Test PDF: ' . $this->upload->display_errors('', ''));
        ...
        if (!empty($_FILES['drawing_file']['name']) && !empty($plan->drawing_file) && file_exists('./uploads/layout_plan/drawing/' . $plan->drawing_file)) {
            @unlink('./uploads/layout_plan/drawing/' . $plan->drawing_file);
        }

        if (!empty($_FILES['layout_photo']['name']) && !empty($plan->layout_photo) && file_exists('./uploads/layout_plan/photo/' . $plan->layout_photo)) {
            @unlink('./uploads/layout_plan/photo/' . $plan->layout_photo);
        }

        if (!empty($_FILES['soil_test_pdf']['name']) && !empty($plan->soil_test_pdf) && file_exists('./uploads/layout_plan/soil/' . $plan->soil_test_pdf)) {
