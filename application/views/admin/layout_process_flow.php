<style>
    ...
    .xc-flow-list { list-style: none; margin: 0; padding: 0; max-width: 720px; }
    .xc-flow-item { position: relative; display: flex; gap: 18px; padding-bottom: 26px; }
    .xc-flow-item:last-child { padding-bottom: 0; }

    .xc-flow-rail { display: flex; flex-direction: column; align-items: center; flex-shrink: 0; }

    .xc-flow-dot {
        width: 42px; height: 42px; border-radius: 50%;
        display: flex; align-items: center; justify-content: center;
        font-size: 18px; color: #fff; background: #cbd3db; z-index: 1;
    }
    ...
    .xc-flow-line { width: 3px; flex: 1; background: var(--xc-border); margin-top: 4px; }
    .xc-flow-item.done .xc-flow-line { background: var(--xc-teal); }

    .xc-flow-card {
        flex: 1; background: #fff; border: 1px solid var(--xc-border);
        border-radius: 14px; box-shadow: 0 2px 10px rgba(20, 40, 60, .04); padding: 16px 18px;
    }

    .xc-flow-card.locked { opacity: .55; }

    .xc-flow-card-top { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; flex-wrap: wrap; }
    .xc-flow-stage { font-weight: 800; color: var(--xc-navy); font-size: 1rem; }
    .xc-flow-member { color: #7c8798; font-size: .8rem; margin-top: 2px; }

    .xc-pill {
        display: inline-flex; align-items: center; gap: 5px; border-radius: 999px;
        padding: 5px 11px; font-size: .74rem; font-weight: 700; white-space: nowrap;
    }
    ...
    .xc-approval-row {
        ...
        border-top: 1px dashed var(--xc-border); flex-wrap: wrap; font-size: .78rem; color: #7c8798; font-weight: 700;
    }

    .xc-approval-row i { color: var(--xc-purple); }

    .xc-btn-sm {
        border: none; border-radius: 8px; padding: 7px 14px; font-size: .8rem; font-weight: 700;
        display: inline-flex; align-items: center; gap: 5px; text-decoration: none;
        background: #f4f6f9; color: #4b5768 !important; margin-top: 12px;
    }

    .xc-empty-flow { text-align: center; padding: 50px 16px; color: #94a0ad; }
</style>
    ...
    <div class="page-content">
        <div class="xc-flow-head">
            ...
            <div style="display:flex; gap:10px; align-items:center;">
                <?php if (!empty($scopes)) { ?>
                    <select class="xc-scope-select" onchange="window.location='<?= base_url('index.php/layout_member/layout_process_flow/'); ?>' + this.value">
                ...
                <a href="<?= base_url('index.php/layout_member/layout_process'); ?>" class="xc-btn-sm">
        ...
        <?php if (empty($flow)) { ?>
            <div class="xc-empty-flow">No layout process flow has started for any client yet.</div>
        <?php } else { ?>
                    ...
                    <li class="xc-flow-item <?= $line_done ? 'done' : ''; ?>">
                        ...
                        <div class="xc-flow-card <?= $state === 'Locked' ? 'locked' : ''; ?>">
                            ...
                            <?php if ($card->report) { ?>
                                ...
                                <a href="<?= base_url('index.php/layout_member/layout_process'); ?>" class="xc-btn-sm">
                                    <i class="bx bx-show"></i> View in Table
                                </a>
                            <?php } ?>
                        </div>
                    </li>
